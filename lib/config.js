...
const CCM_DIR = expandHome(process.env.HH_CCM_DIR || '~/.config/ccm');
...
// ---------- 导入：ccm 三个 conf ----------
function confLines(file) {
  return readText(file, '').split('\n').map((l) => l.replace(/^\s*#.*$/, '').replace(/\s+#.*$/, '').trim()).filter(Boolean).map((l) => l.split(/\s+/));
}
...
function parseExtra(extra) {
  ...
  for (const kv of (extra || '').split(',')) {
    if (!kv) continue;
    const i = kv.indexOf('=');
    if (i <= 0) throw new HHError('bad_extra', `extra 项应为 K=V: ${kv}`);
...
function importCcm(cfg, dir, opts) {
  opts = opts || {};
  dir = dir || CCM_DIR;
  const out = { dir, gateways: [], profiles: [], roles: [], skipped: [] };
  const gwFile = path.join(dir, 'gateways.conf');
  const pfFile = path.join(dir, 'profiles.conf');
  const roFile = path.join(dir, 'roles.conf');
  if (!fs.existsSync(gwFile) && !fs.existsSync(pfFile) && !fs.existsSync(roFile)) throw new HHError('no_ccm', `目录里没有 gateways.conf / profiles.conf / roles.conf: ${dir}`);
  for (const f of confLines(gwFile)) {
    const [name, url, auth, secret, extra] = f;
    if (!name || !url || !auth || !secret) { out.skipped.push(`gateway 行字段不全: ${f.join(' ')}`); continue; }
    if (cfg.gateways[name]) { out.gateways.push({ name, status: 'exists' }); continue; }
    const g = { url, auth, secret };
    const env = parseExtra(extra);
    if (Object.keys(env).length) g.env = env;
    try { validateGateway(name, g); } catch (e) { out.skipped.push(e.message); continue; }
    if (!opts.dryRun) cfg.gateways[name] = g;
    out.gateways.push({ name, status: 'added', url });
  }
  for (const f of confLines(pfFile)) {
    const [name, gateway, model, extra] = f;
    if (!name || !gateway || !model) { out.skipped.push(`profile 行字段不全: ${f.join(' ')}`); continue; }
    if (cfg.profiles[name]) { out.profiles.push({ name, status: 'exists' }); continue; }
    const p = { gateway, model };
    const env = parseExtra(extra);
    if (Object.keys(env).length) p.env = env;
    if (!cfg.gateways[gateway] && !out.gateways.find((g) => g.name === gateway)) { out.skipped.push(`profile ${name} 引用的网关不存在: ${gateway}`); continue; }
    if (!opts.dryRun) cfg.profiles[name] = p;
    out.profiles.push({ name, status: 'added', gateway, model });
  }
  for (const f of confLines(roFile)) {
    const [role, profile, template] = f;
    if (!role || !profile) continue;
    const prof = profile === 'claude' || profile === '-' ? OFFICIAL : profile;
    if (cfg.roles[role]) { out.roles.push({ role, status: 'exists' }); continue; }
    const r = { profile: prof, template: template || `${role}.md`, autonomy: (ROLE_DEFAULTS[role] || {}).autonomy || 'full' };
    if (ROLE_DEFAULTS[role]) r.desc = ROLE_DEFAULTS[role].desc;
    if (!opts.dryRun) cfg.roles[role] = r;
    out.roles.push({ role, status: 'added', profile: prof });
  }
  return out;
}
...
function init(opts) {
  ...
  const result = { config: CONFIG_FILE, claude: claudeBin, herdr: which('herdr'), ccm: null, aliases: null, roles_defaulted: false };
  if (fs.existsSync(path.join(CCM_DIR, 'profiles.conf')) || fs.existsSync(path.join(CCM_DIR, 'gateways.conf'))) {
    result.ccm = importCcm(cfg, CCM_DIR, {});
  }
...
module.exports = {
  ROOT, HH_BIN, CONFIG_DIR, STATE_DIR, CONFIG_FILE, PROMPTS_DIR, REPO_PROMPTS, CCM_DIR, ALIAS_FILES,
  OFFICIAL, AUTONOMY, AUTH, MODEL_ENV_KEYS, GATEWAY_ENV_KEYS, DEFAULT_COMMON_ENV, ROLE_DEFAULTS, DEFAULT_FOOTER, REPORT_SCHEMA,
  ...
  installPrompts, importCcm, importAliases, parseAliasLine, init,
};
