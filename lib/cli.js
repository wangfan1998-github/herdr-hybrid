...
const USAGE = `hh — herdr-hybrid v${api.version}：一份配置管一批「网关 + key + 模型」的 Claude Code profile；聪明的当 Leader，便宜的干活

开始
  hh init [--force]                      发现 claude；自动导入 ~/.config/ccm/*.conf 和 ~/.shell_aliases 里的 xxxcc alias；生成角色
  hh doctor [--net] [--all]              体检；--net 对角色用到的 profile 真发一次 pong（--all 全部 profile）
用 profile 启动 Claude Code（替代 xxxcc alias）
  hh claude                              Leader 模式：用 config.leader 启动，并把 Leader 协议注入 system prompt（说什么都按协议拆解派发）
  hh claude <profile> [claude 参数...]   普通模式：注入该 profile 的网关/密钥/模型后启动交互式 Claude Code（= 以前的 xxxcc）
  hh claude --leader <profile>           指定 profile 的 Leader 模式；--dry-run 只打印命令不启动
  hh profiles aliases                    输出 alias <profile>cc='hh claude <profile>'，eval "$(hh profiles aliases)" 沿用旧习惯
...
配置
  ...
  hh profiles import-aliases [--dry-run] [FILE...]      从 shell alias 导入（默认扫 ~/.shell_aliases ~/.zshrc ~/.bashrc）
  hh profiles import-ccm [--dry-run] [DIR]              从旧 ccm 的 gateways/profiles/roles.conf 导入（默认 ~/.config/ccm）
...
CMD.init = async (argv) => {
  const { opts } = parseArgs(argv, { force: { type: 'bool' } });
  ...
  out(r, () => {
    ...
    if (r.ccm) showImport(r.ccm, '导入 ccm 配置');
    if (r.aliases) showImport(r.aliases, '导入 shell alias');
    say(`网关 ${r.gateways.length} 个，profile ${r.profiles.length} 个（含 official），Leader 默认: ${r.leader}`);
    say('角色分工:');
    ...
    if (r.roles_defaulted) say('  （没有导入到角色，全部默认 official；用 hh roles set <role> --profile <便宜的> 分工）');
    if (r.prompts.length) say(`模板已复制到 ${shortHome(config.PROMPTS_DIR)}/`);
    say('下一步: hh doctor  →  hh profiles test <name>  →  eval "$(hh profiles aliases)"  →  hh install-mcp（可选）');
  });
};
