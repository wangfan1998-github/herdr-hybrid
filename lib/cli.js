...
配置（端点 gateway = 地址 + 鉴权 + 密钥，只存一次；profile = 端点 + 模型；role = 给 Leader 用的分工）
  ...
  hh viewer [auto|herdr|none]            Leader 和 worker 用不用 herdr 窗口：auto = 装了 herdr 就用（默认）· herdr = 必须 · none = 从不
Leader 接入
...
CMD.claude = async (argv) => {
  if (argv[0] === '-h' || argv[0] === '--help') {
    say('用法: hh claude [--leader] [--herdr | --no-herdr] [--dry-run] [profile] [claude 参数...]');
