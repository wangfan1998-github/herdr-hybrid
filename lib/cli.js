...
用 profile 启动 Claude Code
  hh claude                              Leader 模式：用 config.leader 启动并注入 Leader 协议，开在 herdr 的新 tab 里（没装 herdr / Claude Code 会先问你要不要装）
  hh claude <profile> [claude 参数...]   普通模式：注入该 profile 的端点/密钥/模型，在当前终端启动交互式 Claude Code
  hh claude --leader <profile>           指定 profile 的 Leader 模式
      --no-herdr / --herdr               留在当前终端（派出的 worker 也不开窗口）/ 普通模式也开进 herdr；--dry-run 只打印不启动
...
配置（gateway = 一个 API 端点 + 它的密钥；profile = 端点 + 模型；role = 给 Leader 用的分工）
  ...
  hh viewer [auto|herdr|none]            Leader 和 worker 用不用 herdr 窗口：auto = 装了就用（默认）· herdr = 必须 · none = 从不
Leader 接入
...
CMD.claude = async (argv) => {
  ...
  argv = argv.filter((a) => a !== '--herdr' && a !== '--no-herdr');
  ...
  const lopts = { leader, herdr: herdrOpt };
  if (dry) {
  ...
  const offer = async (what, question, after) => {
    ...
    if (r.ok) { say(`✓ ${what} 已安装: ${r.path}`); return after(); }
  ...
  if (plan.missing === 'herdr' && tty) {
    ...
    else { say('好，这次留在当前终端。以后不再问：hh viewer none；只跳过这一次：hh claude --no-herdr'); plan.hint = null; }
  }
  if (plan.hint) process.stderr.write(`hh: ${plan.hint}\n`);
  if (plan.launch === 'pane') say('# hh: 已经在 herdr 里，原地启动；派出的 worker 各开一个 tab');
  if (plan.launch === 'herdr') say(`# hh: 开到 herdr 工作区 ${plan.workspace} 的新 tab（${plan.reason}）…`);
...
CMD.install = async (argv) => {
  ...
  return out(r, () => {
    ...
    if (r.ok) {
      say(`✓ ${name} 已安装: ${r.path}`);
      const dir = require('path').dirname(r.path);
      if (!(process.env.PATH || '').split(require('path').delimiter).includes(dir)) say(`注意: ${dir} 不在 PATH 里，加到 shell 配置后重开终端`);
