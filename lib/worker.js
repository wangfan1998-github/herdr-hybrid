...
const STDERR_NOISE = /\[claude-code:unrecognized_model\]|connectors are disabled because ANTHROPIC_API_KEY/;
...
async function main(id) {
  ...
  const finish = (code, signal, spawnError) => {
    ...
    if (spawnError) error = spawnError;
      ...
      // stderr 里有两类对排障没用的常驻噪音：非 Anthropic 模型名一律触发的 unrecognized_model、设了网关后 connectors 被关的提示。
      // 过滤掉，并把回复里的 API Error 放到最前面——那才是真因（地址 / 密钥 / 模型不存在）。
      const tail = tailLines(path.join(dir, 'stderr.log'), 8).filter((l) => l.trim() && !STDERR_NOISE.test(l)).slice(-3).join(' | ');
      const apiErr = /^\s*API Error/i.test(fin.final || '') ? truncate(String(fin.final).trim().split('\n')[0], 200) : '';
      error = `exit ${code}${signal ? ` (${signal})` : ''}${apiErr ? `: ${apiErr}` : ''}${tail ? `${apiErr ? ' | ' : ': '}${truncate(tail, 300)}` : ''}`;
    }
