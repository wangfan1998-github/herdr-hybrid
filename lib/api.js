...
function decideLaunch(c, opts) {
  ...
  const want = opts.herdr === false ? 'none' : forced ? 'herdr' : (opts.leader ? config.effectiveViewer(c) : 'none');
...
function doctor() {
  ...
  const vNote = viewer === 'none' ? '（viewer=none：Leader 与 worker 都不开窗口）' : hRun ? '（hh claude 开在 herdr 里，worker 各一个 tab）' : '（hh claude 会自动启动 server）';
