...
function launchPlan(profileName, extra, opts) {
  ...
  return { resolved, env, argv, leader: !!opts.leader, prompt };
}
...
function launch(profileName, extra, opts) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('exit', (code, signal) => resolve(code == null ? (signal ? 128 : 1) : code));
  });
}
...
function importCcm(dir, opts) {
  opts = opts || {};
  const c = cfg();
  const r = config.importCcm(c, dir, { dryRun: !!opts.dryRun });
  if (!opts.dryRun) config.save(c);
  r.dry_run = !!opts.dryRun;
  return r;
}
...
function doctor() {
  ...
  add('claude', !!bin, bin ? `${bin} · ${claudeVersion(bin) || '?'}` : `找不到 ${c.claude.bin}`, bin ? '' : '安装 Claude Code 或改 config.claude.bin');
  ...
  add('gateways', gwNames.length > 0 ? true : null, gwNames.length ? gwNames.join(', ') : '（没有端点，只有 official）', gwNames.length ? '' : 'hh setup（交互式）/ hh gateways set / 迁移: hh profiles import-aliases | import-ccm');
  ...
  add('profiles', pNames.length > 1, `${pNames.length} 个：${pNames.slice(0, 12).join(', ')}${pNames.length > 12 ? ' …' : ''}`, pNames.length > 1 ? '' : 'hh setup / hh profiles set / 迁移: import-aliases | import-ccm');
  ...
  add('herdr', herdr.installed() ? herdr.available() : null, herdr.installed() ? (herdr.available() ? '运行中（worker 会开在 tab 里）' : '已安装但未运行（worker 后台跑）') : '未安装（可选）', '');
...
function init(opts) {
  ...
  return {
    config: r.config, claude: r.claude, herdr: r.herdr, ccm: r.ccm, aliases: r.aliases, roles_defaulted: r.roles_defaulted, prompts: r.prompts,
...
module.exports = {
  ...
  launch, launchPlan, envOf,
  gateways, gatewaysSet, gatewaysRm,
  profiles, profilesShow, profilesSet, profilesRm, profilesTest, profilesAliases, importAliases, importCcm,
