...
function doctor() {
  ...
  add('gateways', gwNames.length > 0 ? true : null, gwNames.length ? gwNames.join(', ') : '（没有网关，只有 official）', gwNames.length ? '' : 'hh gateways set / hh profiles import-aliases / import-ccm');
  ...
  add('profiles', pNames.length > 1, `${pNames.length} 个：${pNames.slice(0, 12).join(', ')}${pNames.length > 12 ? ' …' : ''}`, pNames.length > 1 ? '' : 'hh profiles set / import-aliases / import-ccm');
